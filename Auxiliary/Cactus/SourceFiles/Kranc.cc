...
static bool use_psync, psync_only, driver_bc_aliased;
...
void init_parameters() {
    ...
    tmp = (const CCTK_INT *)CCTK_ParameterGet("use_psync", "Cactus", &type);
    if(tmp == 0 or *tmp == 0)
        use_psync = false;
    else
        use_psync = true;

    tmp = (const CCTK_INT *)CCTK_ParameterGet("psync_only", "Cactus", &type);
    if(tmp == 0 or *tmp == 0)
        psync_only = false;
    else
        psync_only = true;
    ...
    if(CCTK_IsFunctionAliased("Driver_SelectVarForBC")) {
        ...
        //CCTK_ERROR("Cactus::use_psync = true, but Driver_SelectVarForBC is unavailable");
    }
}

CCTK_INT KrancBdy_SelectVarForBC(
...
CCTK_INT KrancBdy_SelectGroupForBC(
    const CCTK_POINTER_TO_CONST cctkGH_,
    const CCTK_INT faces,
    const CCTK_INT width,
    const CCTK_INT table_handle,
