...
static void Laplace_relax_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL phirhsL CCTK_ATTRIBUTE_UNUSED = mu*(PDstandard11phi + 
...
extern "C" void Laplace_relax(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "Laplace_relax", 2, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "Laplace_relax", 1, 1, 0);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "Laplace_relax", 2, 2, 0);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, Laplace_relax_Body);
  if (verbose > 1)
  {
