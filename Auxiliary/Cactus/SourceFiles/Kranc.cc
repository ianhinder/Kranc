...
void GetBoundaryInfo(
    cGH const *restrict const cctkGH, int const *restrict const cctk_ash,
    int const *restrict const cctk_lsh, int const *restrict const cctk_bbox,
    int const *restrict const cctk_nghostzones, int *restrict const imin,
    int *restrict const imax, int *restrict const is_symbnd,
    int *restrict const is_physbnd, int *restrict const is_ipbnd) {
  CCTK_INT bbox[6];
  CCTK_INT nboundaryzones[6];
  CCTK_INT is_internal[6];
  CCTK_INT is_staggered[6];
      ...
      if (is_ipbnd[index]) {
        ...
        if (is_symbnd[index]) {
          ...
          if (npoints != cctk_nghostzones[dir]) {
            CCTK_WARN(1, "The number of symmetry points is different from the "
                         "number of ghost points; this is probably an error");
          }
        }
      }
...
void LoopOverEverything(cGH const *restrict const cctkGH,
                        Kranc_Calculation const calc) {
...
void LoopOverBoundary(cGH const *restrict const cctkGH,
                      Kranc_Calculation const calc) {
  DECLARE_CCTK_ARGUMENTS
    ...
    for (dir2 = -1; dir2 <= +1; dir2++) {
      for (dir1 = -1; dir1 <= +1; dir1++) {
        ...
        if (have_bnd && all_physbnd) {
          ...
          calc(cctkGH, old_dir, old_face, normal, tangentA, tangentB, bmin,
               bmax, 0, NULL);
        }
      }
    }
...
void LoopOverBoundaryWithGhosts(cGH const *restrict const cctkGH,
                                Kranc_Calculation const calc) {
  DECLARE_CCTK_ARGUMENTS
...
void LoopOverInterior(cGH const *restrict const cctkGH,
                      Kranc_Calculation const calc) {
  DECLARE_CCTK_ARGUMENTS
...
void GroupDataPointers(cGH const *restrict const cctkGH, const char *group_name,
  ...
  for (int v = 0; v < nvars; v++) {
    ptrs[v] =
        (CCTK_REAL const *)CCTK_VarDataPtrI(cctkGH, 0 /* timelevel */, v1 + v);
  }
}
