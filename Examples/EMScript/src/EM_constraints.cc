...
extern "C" void EM_constraints_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "EMScript::CB_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for EMScript::CB_group.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "EMScript::CEl_group","flat");
  if (ierr < 0)
...
extern "C" void EM_constraints(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "EMScript::CEl_group",
    "EMScript::El_group"};
  AssertGroupStorage(cctkGH, "EM_constraints", 4, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "EM_constraints", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "EM_constraints", 2, 2, 2);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, EM_constraints_Body);
  if (verbose > 1)
  {
