...
static void euler_conserved_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
...
extern "C" void euler_conserved(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "Euler::S_group",
    "Euler::v_group"};
  AssertGroupStorage(cctkGH, "euler_conserved", 6, groups);
  
  
  LoopOverEverything(cctkGH, euler_conserved_Body);
  if (verbose > 1)
  {
