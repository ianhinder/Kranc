...
extern "C" void Laplace_initial_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Laplace::phi_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Laplace::phi_group.");
  return;
}

static void Laplace_initial_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL phiL CCTK_ATTRIBUTE_UNUSED = IfThen(10000000000*fabs(-yL + 
      Ly) < 1,phi0,0);
    /* Copy local copies back to grid functions */
    phi[index] = phiL;
  }
...
extern "C" void Laplace_initial_boundary(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "grid::coordinates",
    "Laplace::phi_group"};
  AssertGroupStorage(cctkGH, "Laplace_initial_boundary", 2, groups);
  ...
  LoopOverBoundary(cctkGH, Laplace_initial_boundary_Body);
  if (verbose > 1)
  {
