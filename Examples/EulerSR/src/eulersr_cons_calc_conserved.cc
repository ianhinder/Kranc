...
static void eulersr_cons_calc_conserved_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL Wx CCTK_ATTRIBUTE_UNUSED = 1 - pow(v1L,2) - pow(v2L,2) - 
      pow(v3L,2);
    
    WL = pow(Wx,-0.5);
    
    pL = epsiL*rhoL*(-1 + gamma);
    
    hL = 1 + epsiL + pL*pow(rhoL,-1);
    
    DenL = rhoL*WL;
    
    CCTK_REAL S1L CCTK_ATTRIBUTE_UNUSED = hL*rhoL*v1L*pow(WL,2);
    
    CCTK_REAL S2L CCTK_ATTRIBUTE_UNUSED = hL*rhoL*v2L*pow(WL,2);
    
    CCTK_REAL S3L CCTK_ATTRIBUTE_UNUSED = hL*rhoL*v3L*pow(WL,2);
    
    CCTK_REAL tauL CCTK_ATTRIBUTE_UNUSED = -DenL - pL + hL*rhoL*pow(WL,2);
    /* Copy local copies back to grid functions */
    Den[index] = DenL;
    h[index] = hL;
    p[index] = pL;
...
extern "C" void eulersr_cons_calc_conserved(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "EulerSR::v_group",
    "EulerSR::W_group"};
  AssertGroupStorage(cctkGH, "eulersr_cons_calc_conserved", 9, groups);
  
  
  LoopOverEverything(cctkGH, eulersr_cons_calc_conserved_Body);
  if (verbose > 1)
  {
