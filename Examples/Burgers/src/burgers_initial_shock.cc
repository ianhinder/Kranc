...
static void burgers_initial_shock_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL uL CCTK_ATTRIBUTE_UNUSED = uL0 + (-uL0 + 
      uR0)*StepFunction(-0.5 + xL);
    /* Copy local copies back to grid functions */
    u[index] = uL;
  }
...
extern "C" void burgers_initial_shock(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "grid::coordinates",
    "Burgers::u_group"};
  AssertGroupStorage(cctkGH, "burgers_initial_shock", 2, groups);
  
  
  LoopOverEverything(cctkGH, burgers_initial_shock_Body);
  if (verbose > 1)
  {
