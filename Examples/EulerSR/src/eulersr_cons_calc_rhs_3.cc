...
static void eulersr_cons_calc_rhs_3_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    const CCTK_REAL PDplus3tauFlux CCTK_ATTRIBUTE_UNUSED = PDplus3(&tauFlux[index]);
...
extern "C" void eulersr_cons_calc_rhs_3(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "eulersr_cons_calc_rhs_3", 8, groups);
  
  EnsureStencilFits(cctkGH, "eulersr_cons_calc_rhs_3", 1, 1, 1);
  
  LoopOverInterior(cctkGH, eulersr_cons_calc_rhs_3_Body);
  if (verbose > 1)
  {
