...
extern "C" void initial_sine_calc_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "SimpleWaveScript::phi_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for SimpleWaveScript::phi_group.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "SimpleWaveScript::pi_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for SimpleWaveScript::pi_group.");
  return;
}

static void initial_sine_calc_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
...
extern "C" void initial_sine_calc(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "grid::coordinates",
    "SimpleWaveScript::phi_group",
    "SimpleWaveScript::pi_group"};
  AssertGroupStorage(cctkGH, "initial_sine_calc", 3, groups);
  ...
  LoopOverEverything(cctkGH, initial_sine_calc_Body);
  if (verbose > 1)
  {
