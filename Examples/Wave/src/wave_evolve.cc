...
extern "C" void wave_evolve(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "Wave::evolved",
    "Wave::evolvedrhs"};
  AssertGroupStorage(cctkGH, "wave_evolve", 2, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "wave_evolve", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "wave_evolve", 2, 2, 2);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, wave_evolve_Body);
  if (verbose > 1)
  {
