...
extern "C" void wave_calc_norm_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Wave::norms","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Wave::norms.");
  return;
}

static void wave_calc_norm_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL VL2squared CCTK_ATTRIBUTE_UNUSED = pow(phiL,2) + pow(piL,2);
    
    CCTK_REAL VL2L CCTK_ATTRIBUTE_UNUSED = pow(VL2squared,0.5);
    
    CCTK_REAL VDPsquared CCTK_ATTRIBUTE_UNUSED = pow(piL,2) + 
      pow(PDplus1phi,2) + pow(PDplus2phi,2) + pow(PDplus3phi,2);
    
    CCTK_REAL VDPL CCTK_ATTRIBUTE_UNUSED = pow(VDPsquared,0.5);
    
    CCTK_REAL EL2squared CCTK_ATTRIBUTE_UNUSED = pow(phiErrorL,2) + 
      pow(piErrorL,2);
    
    CCTK_REAL EL2L CCTK_ATTRIBUTE_UNUSED = pow(EL2squared,0.5);
    /* Copy local copies back to grid functions */
    EL2[index] = EL2L;
    VDP[index] = VDPL;
    VL2[index] = VL2L;
...
extern "C" void wave_calc_norm(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "Wave::evolved",
    "Wave::norms"};
  AssertGroupStorage(cctkGH, "wave_calc_norm", 3, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "wave_calc_norm", 1, 1, 1);
      break;
    }
  ...
  LoopOverInterior(cctkGH, wave_calc_norm_Body);
  if (verbose > 1)
  {
