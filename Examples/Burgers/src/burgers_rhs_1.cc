...
extern "C" void burgers_rhs_1_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Burgers::u_grouprhs","flat");
  if (ierr < 0)
...
extern "C" void burgers_rhs_1(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "Burgers::uF_group",
    "Burgers::u_grouprhs"};
  AssertGroupStorage(cctkGH, "burgers_rhs_1", 2, groups);
  
  EnsureStencilFits(cctkGH, "burgers_rhs_1", 1, 1, 1);
  
  LoopOverInterior(cctkGH, burgers_rhs_1_Body);
  if (verbose > 1)
  {
