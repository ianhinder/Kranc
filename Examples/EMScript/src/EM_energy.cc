...
extern "C" void EM_energy_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "EMScript::rho_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for EMScript::rho_group.");
  return;
}

static void EM_energy_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL rhoL CCTK_ATTRIBUTE_UNUSED = 0.5*(pow(B1L,2) + pow(B2L,2) + 
      pow(B3L,2) + pow(El1L,2) + pow(El2L,2) + pow(El3L,2));
    /* Copy local copies back to grid functions */
    rho[index] = rhoL;
  }
...
extern "C" void EM_energy(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "EMScript::El_group",
    "EMScript::rho_group"};
  AssertGroupStorage(cctkGH, "EM_energy", 3, groups);
  ...
  LoopOverEverything(cctkGH, EM_energy_Body);
  if (verbose > 1)
  {
