...
static void advect_flux_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
...
extern "C" void advect_flux(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "Advect::rho_group",
    "Advect::v_group"};
  AssertGroupStorage(cctkGH, "advect_flux", 3, groups);
  
  
  LoopOverEverything(cctkGH, advect_flux_Body);
  if (verbose > 1)
  {
