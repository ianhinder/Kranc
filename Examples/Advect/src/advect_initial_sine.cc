...
static void advect_initial_sine_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL rhoL CCTK_ATTRIBUTE_UNUSED = 1 + amp*sin(2*xL*Pi);
    /* Copy local copies back to grid functions */
    rho[index] = rhoL;
    v1[index] = v1L;
    v2[index] = v2L;
...
extern "C" void advect_initial_sine(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "Advect::rho_group",
    "Advect::v_group"};
  AssertGroupStorage(cctkGH, "advect_initial_sine", 3, groups);
  
  
  LoopOverEverything(cctkGH, advect_initial_sine_Body);
  if (verbose > 1)
  {
