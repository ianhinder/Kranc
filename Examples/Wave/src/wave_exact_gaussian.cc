...
static void wave_exact_gaussian_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL rEps CCTK_ATTRIBUTE_UNUSED = pow(1.e-24 + pow(rL,4),0.25);
    
    CCTK_REAL phiExactL CCTK_ATTRIBUTE_UNUSED = 
      pow(rEps,-1)*(-(exp(-(pow(nSigma,-2)*pow(-rL + t + t0,2)))*pow(-rL + t 
      + t0,3)) + exp(-(pow(nSigma,-2)*pow(rL + t + t0,2)))*pow(rL + t + 
      t0,3));
    
    CCTK_REAL piExactL CCTK_ATTRIBUTE_UNUSED = 
      pow(rEps,-1)*(exp(-(pow(nSigma,-2)*pow(-rL + t + t0,2)))*(-3*pow(-rL + 
      t + t0,2) + 2*pow(nSigma,-2)*pow(-rL + t + t0,4)) + 
      exp(-(pow(nSigma,-2)*pow(rL + t + t0,2)))*(3*pow(rL + t + t0,2) - 
      2*pow(nSigma,-2)*pow(rL + t + t0,4)));
    /* Copy local copies back to grid functions */
...
extern "C" void wave_exact_gaussian(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "Wave::exact",
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "wave_exact_gaussian", 2, groups);
  ...
  LoopOverEverything(cctkGH, wave_exact_gaussian_Body);
  if (verbose > 1)
  {
