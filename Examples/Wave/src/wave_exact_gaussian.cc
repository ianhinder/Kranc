...
static void wave_exact_gaussian_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED rEps = pow(1.e-24 + QAD(rL),0.25);
    
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED phiExactL = (-1.*CUB(-1.*rL + t + 
      ToReal(t0))*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(-1.*rL + t + 
      ToReal(t0))) + CUB(rL + t + 
      ToReal(t0))*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(rL + t + 
      ToReal(t0))))*INV(rEps);
    
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED piExactL = 
      INV(rEps)*(INV(SQR(ToReal(nSigma)))*(2.*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(-1.*rL 
      + t + ToReal(t0)))*QAD(-1.*rL + t + ToReal(t0)) - 
      2.*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(rL + t + 
      ToReal(t0)))*QAD(rL + t + ToReal(t0))) - 
      3.*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(-1.*rL + t + 
      ToReal(t0)))*SQR(-1.*rL + t + ToReal(t0)) + 
      3.*exp(-1.*INV(SQR(ToReal(nSigma)))*SQR(rL + t + 
