  ...
  for (int tk = tiled_imin[2]; tk < tiled_imax[2]; tk += tile_size_l[2])
  {
    for (int tj = tiled_imin[1]; tj < tiled_imax[1]; tj += tile_size_l[1])
    {
      for (int ti = tiled_imin[0]; ti < tiled_imax[0]; ti += tile_size_l[0])
      {
        ...
        kd.tile_imin[0] = max(kd_coarse.imin[0], tk);
        kd.tile_imax[0] = min(kd_coarse.imax[0], tk + tile_size_l[2]);
        kd.tile_imin[1] = max(kd_coarse.imin[1], tj);
        kd.tile_imax[1] = min(kd_coarse.imax[1], tj + tile_size_l[1]);
        kd.tile_imin[2] = max(kd_coarse.imin[2], ti);
        kd.tile_imax[2] = min(kd_coarse.imax[2], ti + tile_size_l[0]);
...
void @THORN_NAME@_TiledLoopOverEverything(
  cGH const * restrict const cctkGH,
  void (calc)(const cGH* restrict const cctkGH,
              const KrancData & restrict kd))
{
  ...
  @THORN_NAME@_TiledLoop(cctkGH, kd_coarse, calc);
}
