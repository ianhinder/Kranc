...
static void eulerauto_initial_shock_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL rhoL CCTK_ATTRIBUTE_UNUSED = rhoL0 + (-rhoL0 + 
      rhoR0)*StepFunction(-0.5 + xL);
    
    CCTK_REAL v1L CCTK_ATTRIBUTE_UNUSED = vL0 + (-vL0 + 
      vR0)*StepFunction(-0.5 + xL);
    
    CCTK_REAL v2L CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL v3L CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL pL CCTK_ATTRIBUTE_UNUSED = pL0 + (-pL0 + 
      pR0)*StepFunction(-0.5 + xL);
    /* Copy local copies back to grid functions */
    p[index] = pL;
    rho[index] = rhoL;
    v1[index] = v1L;
...
extern "C" void eulerauto_initial_shock(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "EulerAuto::rho_group",
    "EulerAuto::v_group"};
  AssertGroupStorage(cctkGH, "eulerauto_initial_shock", 4, groups);
  
  
  LoopOverEverything(cctkGH, eulerauto_initial_shock_Body);
  if (verbose > 1)
  {
