...
static void wave_exact_sine_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED piconst = 3.1415926535897932385;
    
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED phiExactL = 
      sin(2.*piconst*INV(ToReal(periodicity))*(-1.*t*sqrt(SQR(ToReal(n1)) + 
    ...
    CCTK_REAL CCTK_ATTRIBUTE_UNUSED piExactL = 
      -2.*piconst*cos(2.*piconst*INV(ToReal(periodicity))*(-1.*t*sqrt(SQR(ToReal(n1)) 
