...
extern "C" void euler_rhs_1_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Euler::Den_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Euler::Den_grouprhs.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Euler::En_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Euler::En_grouprhs.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Euler::S_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Euler::S_grouprhs.");
  return;
}

static void euler_rhs_1_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    const CCTK_REAL PDplus1DenF CCTK_ATTRIBUTE_UNUSED = PDplus1(&DenF[index]);
    const CCTK_REAL PDplus1EnF CCTK_ATTRIBUTE_UNUSED = PDplus1(&EnF[index]);
    const CCTK_REAL PDplus1SF1 CCTK_ATTRIBUTE_UNUSED = PDplus1(&SF1[index]);
    const CCTK_REAL PDplus1SF2 CCTK_ATTRIBUTE_UNUSED = PDplus1(&SF2[index]);
    const CCTK_REAL PDplus1SF3 CCTK_ATTRIBUTE_UNUSED = PDplus1(&SF3[index]);
...
extern "C" void euler_rhs_1(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "Euler::SF_group",
    "Euler::S_grouprhs"};
  AssertGroupStorage(cctkGH, "euler_rhs_1", 6, groups);
  
  EnsureStencilFits(cctkGH, "euler_rhs_1", 1, 1, 1);
  
  LoopOverInterior(cctkGH, euler_rhs_1_Body);
  if (verbose > 1)
  {
