...
static void eulersr_cons_calc_intercell_conserved_3_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
...
extern "C" void eulersr_cons_calc_intercell_conserved_3(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "EulerSR::v3_lr_group",
    "EulerSR::W_group"};
  AssertGroupStorage(cctkGH, "eulersr_cons_calc_intercell_conserved_3", 13, groups);
  
  
  LoopOverEverything(cctkGH, eulersr_cons_calc_intercell_conserved_3_Body);
  if (verbose > 1)
  {
