...
static void wave_exact_sine_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL piconst CCTK_ATTRIBUTE_UNUSED = 3.1415926535897932385;
    
    CCTK_REAL phiExactL CCTK_ATTRIBUTE_UNUSED = 
      amplitude*sin(2*piconst*pow(periodicity,-1)*(xL*n1 + yL*n2 + zL*n3 - 
      t*pow(pow(n1,2) + pow(n2,2) + pow(n3,2),0.5)));
    
    CCTK_REAL piExactL CCTK_ATTRIBUTE_UNUSED = 
      -2*amplitude*piconst*cos(2*piconst*pow(periodicity,-1)*(xL*n1 + yL*n2 + 
      zL*n3 - t*pow(pow(n1,2) + pow(n2,2) + 
      pow(n3,2),0.5)))*pow(periodicity,-1)*pow(pow(n1,2) + pow(n2,2) + 
      pow(n3,2),0.5);
    /* Copy local copies back to grid functions */
...
extern "C" void wave_exact_sine(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "Wave::exact",
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "wave_exact_sine", 2, groups);
  ...
  LoopOverEverything(cctkGH, wave_exact_sine_Body);
  if (verbose > 1)
  {
